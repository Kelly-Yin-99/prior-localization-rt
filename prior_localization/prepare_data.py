...
def prepare_behavior(
    ...
    """
    Patched for subgroup decoding (fast/normal/slow):
    - Ensures trials_df, trials_mask, and target masks are all in the SAME trial space (same length).
    - Handles the common case where compute_target_mask returns a mask defined only on the "kept" trials
      (length == trials_mask.sum()) by expanding it back to full length (len(trials_df)).
    """

    if pseudo_ids is None:
        pseudo_ids = [-1]  # -1 is always the actual session

    # ---- NEW: normalize / validate trial-space ----
    trials_df = trials_df.reset_index(drop=True)
    trials_mask = np.asarray(trials_mask).astype(bool)
    n_trials = len(trials_df)
    if len(trials_mask) != n_trials:
        raise ValueError(
            f"prepare_behavior: trials_mask len {len(trials_mask)} != trials_df len {n_trials}"
        )

    def _align_target_mask(target_mask, base_trials_mask, n_trials_total):
        """
        Force target_mask into the same space as trials_df (length == n_trials_total).

        Accepts:
        - target_mask length == n_trials_total  -> ok
        - target_mask length == base_trials_mask.sum() -> expand into full length using base_trials_mask positions

        Raises on any other length, because alignment would be ambiguous.
        """
        tm = np.asarray(target_mask).astype(bool)

        if tm.shape[0] == n_trials_total:
            return tm

        n_kept = int(np.sum(base_trials_mask))
        if tm.shape[0] == n_kept:
            full = np.zeros(n_trials_total, dtype=bool)
            full[base_trials_mask] = tm
            return full

        raise ValueError(
            f"prepare_behavior: target_mask len {tm.shape[0]} does not match "
            f"n_trials {n_trials_total} or trials_mask.sum() {n_kept}"
        )
    ...
    if -1 in pseudo_ids:
        ...
        # might need to mask out certain trials based on target vals
        actual_target_mask = compute_target_mask(actual_target, target)
        # ---- NEW: align target mask into trials_df space ----
        actual_target_mask = _align_target_mask(actual_target_mask, trials_mask, n_trials)
    ...
    for n in range(n_pseudo_sets):
        ...
        for pseudo_id in pseudo_ids:
            if pseudo_id == -1:  # this is the actual session
                ...
                target_mask = compute_target_mask(control_targets, target)
                # ---- NEW: align pseudo target mask into trials_df space ----
                target_mask = _align_target_mask(target_mask, trials_mask, n_trials)
                ...
                if config['estimator'] == sklm.LogisticRegression:
                    ...
                    if sample_pseudo_count > 1:
                        logger.info(
                            f'sampled pseudo sessions {sample_pseudo_count} times to ensure valid target array'
                        )
...
def prepare_behavior(
    ...
    for n in range(n_pseudo_sets):
        ...
        # Neurometrics (still in full space, then slice)
        if compute_neurometrics:
            set_neurometrics = []
            for i in range(len(pseudo_ids)):
                neurometrics = compute_neurometric_prior(set_trials[i], session_id, subject, model, behavior_path)
